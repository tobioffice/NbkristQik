...
bot.onText(/\/report$/, (msg) => {
    try {
        bot.sendMessage(msg.chat.id, '⚠️ Please include your report message after /report\nExample: /report I found a bug here');
    } catch (error) {
...
bot.onText(/\/report (.+)/, (msg, match) => {
    try {
        ...
        bot.sendMessage(process.env.ADMIN_ID || "1329532701", formattedMessage, { parse_mode: 'HTML' })
            .catch(err => console.error('Error forwarding report:', err));

        // Confirm to the user
        bot.sendMessage(msg.chat.id, '✅ Thank you! Your report has been sent to the admin.', { parse_mode: 'HTML' });
    } catch (error) {
        console.error('Error in /report command:', error);
        bot.sendMessage(msg.chat.id, '❌ An error occurred while sending your report. Please try again later.');
    }
});