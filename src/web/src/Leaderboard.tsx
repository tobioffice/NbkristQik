...
export default function Leaderboard() {
   ...
   const [sortBy, setSortBy] = useState<"attendance" | "midmarks">(
      "attendance"
   );
   ...
   //          setUserId(launchParams.tgWebAppData?.user?.id || null);
   //      };
   //    initTelegram();
   // }, []);
   ...
   const fetchLeaderboard = async (pageNum: number, sort: string) => {
      ...
      try {
         ...
         const queryParams = new URLSearchParams({
            ...
            // userId: userId ? userId.toString() : "all",
         });
            ...
            <div className="space-y-3">
               {rest.map((stat, index) => (
                  <div
                     key={stat.roll_no}
                     ref={
                        index + 3 === stats.length - 1 ? lastElementRef : null
                     }
