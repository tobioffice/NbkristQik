        ...
        try {
            function formatAttendanceMessage(data: any): string {
                ...
                for (const sub of data.subjects) {
                    ...
                    msg += `${subjectName.padEnd(9)} │ ${status} │${sub.attended.toString().padStart(2)}/${sub.conducted.toString().padStart(2)} │${lastUpdated.padEnd(4)}\n`;
                }
            ...
            let formattedMessage = "<code>Something went wrong !</code>";
            if (yourDataObject.rollno !== "Not Found") {
                formattedMessage = formatAttendanceMessage(yourDataObject);
            }
        ...
        try {
            function formatMidMarksMessage(data: any): string {
                ...
                for (const sub of data.subjects) {
                    ...
                    msg += `${subjectName.padEnd(11)} │ ${type.padEnd(4)} │` +
                        ` ${sub.M1.toString().padStart(2)} ${sub.M2.toString().padStart(2)}` +
            ...
            if (yourDataObject.rollno !== "Not Found") {
                ...
                formattedMessage = formatMidMarksMessage(yourDataObject);
            }
